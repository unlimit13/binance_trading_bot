...
def main():
    ...
    while True:
        ...
        qty, resp_sl, resp_tp = open_position(
            ...
            price=order_price , loss_pct=0.5, gain_pct=0.05, tif=TIF
        )
        ...
        if reason in ("TP", "SL"):
            ...
            tx_lines.append(f"[CLOSE] 이유={reason}, orderId={filled_id}, avg={stats['avg']}, "
                            f"qty={stats['qty']}, realized={stats['realized']:.4f}, fee={stats['fee']:.4f}")
            ...
            if close_order_id:
                ...
                tx_lines.append(f"[CLOSE] 이유=IDLE, orderId={close_order_id}, avg={stats['avg']}, "
                                f"qty={stats['qty']}, realized={stats['realized']:.4f}, fee={stats['fee']:.4f}")
                tx_lines.append(f"[RESULT] Net PnL={profit:.4f} USDT, ROI(margin)={roi_pct}")
            else:
                profit = 0.0
